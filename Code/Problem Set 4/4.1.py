...
def process_name(name, label):
    if name == 'NULL' or not name.isalpha():
        name = label
    return name


def convert_nulls_to_none(d):
    for i in d:
        if d[i] == 'NULL':
            d[i] = None
    return d
...
def remove_extra_descriptions(text):
    start_index = text.find('(')
    if start_index == -1:
        return text
    end_index = text.find(')')
    text = text.replace(text[start_index:end_index+1], '').strip()
    return text
...
def process_file(filename, fields):
    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
            this_data = change_labels(line, FIELDS)
            # Transform Data
            this_data['label'] = remove_extra_descriptions(this_data['label'])
            this_data['name'] = process_name(this_data['name'], this_data['label'])
            this_data = convert_nulls_to_none(this_data)
            if this_data['synonym']:
                this_data['synonym'] = parse_array(this_data['synonym'])
            else:
                this_data['synonym'] = None
            this_data = strip_whitespace(this_data)
            this_data = create_data_structure(this_data)
