...
def process_file(f):
    ...
    with open("{}/{}".format(datadir, f), "r") as html:
        ...
        for tr in table.find_all("tr", {'class': 'dataTDRight'}):
            if tr.contents[2].text == 'TOTAL':
                continue
            this_dictionary = {'courier': info["courier"], 'airport': info["airport"]}
            this_dictionary['year'] = int(tr.contents[1].text)
            this_dictionary['month'] = int(tr.contents[2].text)
            this_dictionary['flights']={
                'domestic': int(tr.contents[3].text.replace(",","")),
                'international': int(tr.contents[4].text.replace(",",""))
            }
