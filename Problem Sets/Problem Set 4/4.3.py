    ...
    with open(filename, "r") as f:
        ...
        for row in reader:
            if row['binomialAuthority_label'] == 'NULL':
                continue
            data[row['rdf-schema#label']] = row['binomialAuthority_label']
        for k in data:
            label = process_label(k)
            data[label] = data.pop(k)
    return data


def update_db(data, db):
    for i in data:
        spider = db.arachnid.find_one({'label': i}, {'_id': 0})
        spider['classification']['binomialAuthority'] = data[i]
        db.arachnid.update({'label': i },{'$set': spider} )
